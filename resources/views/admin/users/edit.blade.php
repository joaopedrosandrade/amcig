...
<main class="app-wrapper">
    <div class="container-fluid">
        <div class="main-breadcrumb d-flex align-items-center my-3 position-relative">
            <h2 class="breadcrumb-title mb-0 flex-grow-1 fs-14">Editar Usuário Administrativo</h2>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                            <form action="{{ route('admin.config.usuarios.update', $admin) }}" method="POST">
                                @csrf
                                @method('PUT')
                                        ...
                                        <div class="mb-3">
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox" id="is_superadmin" name="is_superadmin" value="1" 
                                                       {{ old('is_superadmin', $admin->is_superadmin) ? 'checked' : '' }}
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <div class="alert alert-info">
                                            <h6 class="alert-heading">Informações do Usuário</h6>
                                            <p class="mb-1"><strong>Criado em:</strong> {{ $admin->created_at->format('d/m/Y H:i:s') }}</p>
                                            @if($admin->last_login_at)
                                                <p class="mb-1"><strong>Último acesso:</strong> {{ $admin->last_login_at->format('d/m/Y H:i:s') }} ({{ $admin->last_login_at->diffForHumans() }})</p>
                                            @else
                                                <p class="mb-1"><strong>Último acesso:</strong> Nunca acessou</p>
                                            @endif
                                            @if($admin->updatedBy)
                                                <p class="mb-0"><strong>Última atualização por:</strong> {{ $admin->updatedBy->name }} em {{ $admin->updated_at->format('d/m/Y H:i:s') }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div id="permissions-section">
                                    ...
                                    <div class="row">
                                        @foreach($menus as $menuKey => $menuName)
                                            @php
                                                $permission = $permissions[$menuKey] ?? [];
                                            @endphp
                                                    ...
                                                    <div class="card-body">
                                                        <div class="form-check mb-2">
                                                            <input class="form-check-input permission-checkbox" 
                                                                   ...
                                                                   value="1"
                                                                   {{ old("permissions.{$menuKey}.can_view", $permission['can_view'] ?? false) ? 'checked' : '' }}
                                                        ...
                                                        <div class="form-check mb-2">
                                                            <input class="form-check-input permission-checkbox permission-{{ $menuKey }}" 
                                                                   ...
                                                                   value="1"
                                                                   {{ old("permissions.{$menuKey}.can_create", $permission['can_create'] ?? false) ? 'checked' : '' }}
                                                                   disabled>
                                                        ...
                                                        <div class="form-check mb-2">
                                                            <input class="form-check-input permission-checkbox permission-{{ $menuKey }}" 
                                                                   ...
                                                                   value="1"
                                                                   {{ old("permissions.{$menuKey}.can_update", $permission['can_update'] ?? false) ? 'checked' : '' }}
                                                                   disabled>
                                                        ...
                                                        <div class="form-check">
                                                            <input class="form-check-input permission-checkbox permission-{{ $menuKey }}" 
                                                                   ...
                                                                   value="1"
                                                                   {{ old("permissions.{$menuKey}.can_delete", $permission['can_delete'] ?? false) ? 'checked' : '' }}
                                                                   disabled>
